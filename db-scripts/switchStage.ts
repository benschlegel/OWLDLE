...
let currentFormattedPlayer = liveCurrentName ? newRosterEntry.players.find((p) => p.name === liveCurrentName) : undefined;
...
if (currentFormattedPlayer) {
	...
	const intersection = newRosterEntry.players.filter((p) => livePlayerNames.has(p.name));
...
// nextPlayer: any new-roster player distinct from currentPlayer.
const nextFormattedPlayer = newRosterEntry.players.find((p) => p.name !== currentPlayer.name);
