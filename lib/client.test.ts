...
const FORMAT_CONFIG_8: FormatConfig = { guesses: LOST_GUESSES_8, gameIteration: 2, gameName: 'NAME', maxGuesses: 8, siteUrlShorthand: 'my.url.com' };

const FORMAT_CONFIG_4: FormatConfig = { guesses: WON_GUESSES_4, gameIteration: 1, gameName: 'TEST_NAME', maxGuesses: 8, siteUrlShorthand: 'test.url.com' };

describe('game result', () => {
	test('header correct', () => {
		const formattedResult = formatResult(FORMAT_CONFIG_4);
		expect(formattedResult).toStartWith(
			`${FORMAT_CONFIG_4.gameName} ${FORMAT_CONFIG_4.gameIteration} ${FORMAT_CONFIG_4.guesses.length}/${FORMAT_CONFIG_4.maxGuesses}\n`
		);
	});
	test('footer correct', () => {
		const formattedResult = formatResult(FORMAT_CONFIG_4);
		expect(formattedResult).toEndWith(FORMAT_CONFIG_4.siteUrlShorthand);
	});
	test('won after 4 guesses', () => {
		const formattedResult = formatResult(FORMAT_CONFIG_4);
		const expectedString = getExpectedString(FORMAT_CONFIG_4, '🟩🟥🟥🟥\n🟩🟩🟥🟥\n🟩🟩🟩🟥\n🟩🟩🟩🟩✅');
		expect(formattedResult).toBe(expectedString);
	});
	test('lost after 8 guesses', () => {
		const formattedResult = formatResult(FORMAT_CONFIG_8);
		const expectedString = getExpectedString(FORMAT_CONFIG_8, '🟩🟥🟥🟥\n🟩🟥🟥🟥\n🟩🟩🟩🟥\n🟩🟥🟩🟩\n🟥🟥🟩🟩\n🟩🟥🟩🟩\n🟩🟥🟥🟩\n🟩🟥🟩🟩❌');
		expect(formattedResult).toBe(expectedString);
	});
});
