...
export function StreakDisplay({ stats, onOpenLeaderboard }: { stats: StatsDisplay; onOpenLeaderboard?: () => void }) {
	return (
		<div className="w-full rounded-lg sm:h-[3.7rem] h-[3.1rem] bg-card -mt-4 mb-2 flex items-center justify-between px-8">
			...
			{onOpenLeaderboard && (
				<button onClick={onOpenLeaderboard} className="flex flex-col items-center" type="button">
					<Trophy className="sm:size-4 size-3.5 text-yellow-500" />
					<span className="text-sm font-owl">board</span>
				</button>
			)}
		</div>
