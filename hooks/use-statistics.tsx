...
type Params = { dataset: string; range: TimeframeRange; from: string | null; to: string | null; stage?: string };

async function fetchStatistics({ dataset, range, from, to, stage }: Params): Promise<StatisticsResponse> {
...
export function useStatistics(params: Params) {
	return useQuery({
		queryKey: ['statistics', params.dataset, params.range, params.from, params.to, params.stage ?? 'all'],
